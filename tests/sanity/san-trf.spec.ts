...
test.describe("Sanity tests - Virements interne", () => {
    ...
    test("SAN-TRF-INT-01 - Accès formulaire de virement", async ({ page }) => {
...
test.describe("Sanity tests - Virements compte Tiers", () => {
    let connexionPage: ConnexionPage;
    let navigationPage: NavigationPage;
    let transferPage: TransferPage;
    ...
    // test("SAN-TRF-EXT-02 - Rejet virement montant nul", async ({ page }) => {
    //     await transferPage.virementExterne(individu.accounts[0].id, individu.beneficiaires[0].id, -100, "Virement test");
    //     await expectMessage(transferPage.errorMessage, messagesFixture.error_messages.insufficient_montant);
    // });

    test("SAN-TRF-EXT-03 - Rejet virement solde insuffisant", async ({ page }) => {
        await transferPage.virementExterne(individu.accounts[0].id, individu.beneficiaires[0].name, 100000, "Virement test");
        await expectMessage(transferPage.errorTransfertMessage, messagesFixture.error_messages.insufficient_funds);
    });
});
